...
public class ManageRequestServiceImp implements ManageRequestService{
	...
	public List<RequestObject> passengerGetHistoryList(RequestDTO requestDTO) {
		...
		requestStatusList.add(RequestStatus.DRIVER_ACCEPTED);
		requestStatusList.add(RequestStatus.PAID);
		
		List<ManageRequest> manageRequestList = manageRequestRepository.getPassengerManageRequestByRequestStatus(requestDTO.getAccountId(), requestStatusList);
